...
import {
  type CartaYouErrorCode,
  type CartaYouErrorInterface,
  type CartaYouToken,
} from 'loot-core/types/models/cartayou';
...
function renderError(
  ...
  const error_messages: Partial<Record<CartaYouErrorCode, string>> = {
    ...
    CARTAYOU_SESSION_EXPIRED: t('Session expired. Please try again.'),
...
type CartaYouSetupAccountModalProps = Extract<
  ModalType,
  { name: 'cartayou-setup-account' }
>['options'];

export function CartaYouSetupAccountModal({
  onSuccess,
}: CartaYouSetupAccountModalProps) {
  const { t } = useTranslation();
  const serverURL = useServerURL();

  const [error, setError] = useState<CartaYouErrorInterface | null>(null);
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [isLoggingIn, setIsLoggingIn] = useState(false);
  const [token, setToken] = useState<CartaYouToken | null>(null);
  ...
  const webhookUrl = serverURL ? `${serverURL}/cartayou/sms-webhook` : '';
    ...
    const checkStatus = async () => {
      try {
        const [credResult, smsResult] = await Promise.all([
          send('cartayou-status'),
          send('cartayou-sms-status'),
        ]);
  ...
  const handleGenerateSecret = async () => {
    ...
    try {
      const result = await send('cartayou-sms-setup');
  ...
  const handleLoginWithSaved = async () => {
    ...
    try {
      const loginResult = await send('cartayou-login');
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const configResult = await send('cartayou-configure', {
        username,
        password,
      });
                ...
                {isConfigured && !showNewCredentials ? (
                  <>
                    <Paragraph style={{ fontSize: 15 }}>
                      <Trans>
                        Your Carta You credentials are already configured. Click
                    ...
                    {isLoggingIn ? (
                      <WaitingIndicator
                        message={t(
                          'Logging in to Carta You. This may take a moment...',
                        )}
                      />
                    ) : (
                      <>
                  ...
                  <>
                    <Paragraph style={{ fontSize: 15 }}>
                      <Trans>
                        Enter your Carta You (Advanzia) credentials. Your
                    ...
                    <Warning style={{ marginBottom: 15 }}>
                      <Trans>
                        ...
                        Carta You account. Your credentials are stored on your
                    ...
                    <FormField>
                      <FormLabel
                        title={t('Username')}
                        htmlFor="cartayou-username"
                      />
                      <Input
                        id="cartayou-username"
                        type="text"
                        value={username}
                        onChangeValue={setUsername}
                        placeholder={t('Enter your Carta You username')}
                    ...
                    <FormField style={{ marginTop: 10 }}>
                      <FormLabel
                        title={t('Password')}
                        htmlFor="cartayou-password"
                      />
                      <Input
                        id="cartayou-password"
                        type="password"
                        value={password}
                        onChangeValue={setPassword}
                        placeholder={t('Enter your Carta You password')}
                        disabled={isLoggingIn}
                      />
                    </FormField>
                    ...
                    >
                      ...
                      {showSmsSetup && (
                        <View style={{ padding: 12 }}>
                          ...
                          >
                            <Trans>
                              Carta You requires SMS verification. Set up
